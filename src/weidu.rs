...
pub fn generate_args(weidu_mod: &ModComponent, language: &str) -> Vec<String> {
    format!("{mod_install_path} --force-install {component} --use-lang {game_lang} --language {mod_lang}", mod_install_path = weidu_mod.install_path, component = weidu_mod.component, mod_lang = weidu_mod.lang, game_lang = language).split(' ').map(|x|x.to_string()).collect()
}
    ...
    match weidu_process.output() {
        Ok(output) if !output.status.success() => {
            let mut lines = std::str::from_utf8(&output.stdout)
                .unwrap_or_default()
                .split('\n');
            if let Some(warning) = lines.find(|x| x.starts_with("WARNING:")) {
                log::warn!("Warning found: {}", warning);
            } else {
                for line in lines {
                    log::error!("{}", line);
                }
